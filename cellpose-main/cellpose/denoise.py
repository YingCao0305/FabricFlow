...
def add_noise(lbl, alpha=4, beta=0.7, poisson=0.7, blur=0.7, gblur=1.0, downsample=0.7,
    ...
    diams = diams if diams is not None else 50. * torch.ones(len(lbl), device=device)
    ...
    if iblur.sum() > 0:
        if sigma0 is None:
            ...
            xrand *= gblur
            sigma0 = diams[iblur] / 50. * 5. * torch.from_numpy(xrand).float().to(
                device)
            #(1 + torch.rand(iblur.sum(), device=device))
            if not iso:
                sr = diams[iblur] / 50. * 2 * (1 +
...
def random_rotate_and_resize_noise(data, labels=None, diams=None, poisson=0.7, blur=0.7,
                                   downsample=0.0, beta=0.7, gblur=1.0, diam_mean=50,
    ...
    diams = 50 if diams is None else diams
...
class DenoiseModel():
    ...
    def __init__(self, gpu=False, pretrained_model=False, nchan=1, model_type=None,
                 chan2=False, diam_mean=50., device=None):
        self.nchan = nchan
        ...
        if model_type is not None or (pretrained_model and
            ...
            self.diam_mean = 17. if "nuclei" in pretrained_model_string else 50.
        else:
            if pretrained_model:
                builtin = False
    ...
    def eval(self, x, batch_size=8, channels=None, channel_axis=None, z_axis=None,
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            iterator = trange(nimg, file=tqdm_out,
                              mininterval=50) if nimg > 1 else range(nimg)
            imgs = []
            for i in iterator:
                imgi = self.eval(
            ...
            for c in range(x.shape[-1]):
                rescale0 = rescale * 50. / 17. if c == 1 else rescale
...
if __name__ == "__main__":
    ...
    training_args.add_argument(
        "--diam_mean", default=50., type=float, help=
